...
# função
funcao = sp.lambdify(x, sp.cot(x), 'numpy')

# Gerar valores de x
x_vals = np.linspace(-2 * np.pi, 2 * np.pi, 400)

plano_cartesiano = PlanoCartesiano()
plano_cartesiano.inserir_pontos(valores_eixo_x=x_vals, valores_eixo_y=funcao(x_vals), label=f'Função Cotangente', cor="red")
